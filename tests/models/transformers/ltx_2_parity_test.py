...
class LTX2ParityTest(unittest.TestCase):
    ...
    def test_rope_split_shape(self):
        ...
        # Create coords input.
        # coords shape: [B, 3, num_patches] (?)
        # Wait, how is it called?
        # In transformer_ltx2.py:
        # self.rope(coords)
        # coords is generated by `get_3d_rotary_pos_embed` or similar helper?
        # No, `LTX2VideoTransformer3DModel` calls `self.rope(coords)`.
        # `coords` comes from `prepare_latents` or similar?
        # Actually `LTX2AudioVideoRotaryPosEmbed` expects `coords` as input?
        # Let's check `forward` signature in LTX2AudioVideoRotaryPosEmbed.
        # It takes `latents` (hidden_states) or just `indices`?
        # In `transformer_ltx2.py`:
        # grid = ...
        # freqs = self.rope(grid)
        
        # I need to see how to call it.
        # It takes `grid` of shape (B, 3, num_patches).
